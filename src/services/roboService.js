  ...
  async registrarRobo(interaction, data) {
    try {
      ...
      if (!establecimientos || Object.keys(establecimientos).length === 0) {
        throw new Error('No hay establecimientos configurados. Usa /addrobo para agregar algunos.');
      }
      
      const establecimiento = this.buscarEstablecimiento(establecimientos, data.establecimientoValue);
      ...
      try {
        await RoboQueries.registrarRobo({
        ...
        logger.warn('No se pudo registrar en BD (continuando):', dbError.message);
      }
      
      // Actualizar contadores en memoria y cache
      await this.actualizarContadores(establecimiento, data.exito, participantesIds);
      
      // Crear embed
      const embed = this.crearEmbedRobo(establecimiento, data);
      
      logger.info('Robo registrado', {
  ...
  crearEmbedRobo(establecimiento, data) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '🎯 Resultado', value: data.exito ? '✅ Éxito' : '❌ Fracaso', inline: false }
      )
      .setTimestamp();
